...
def generate_path(
    ...
    codes = (
        [mpltPath.Path.MOVETO]
        + [mpltPath.Path.LINETO]
        * (df_bound[["x", "y"]].to_numpy().shape[0] - 2)
        + [mpltPath.Path.CLOSEPOLY]
    )
    ...
    if buffer > 0:
        ...
        codes = (
            [mpltPath.Path.MOVETO]
            + [mpltPath.Path.LINETO] * (expanded_vertices.shape[0] - 2)
            + [mpltPath.Path.CLOSEPOLY]
        )
...
if has_icecube_package():
    ...
    detector_boundaries = MuonGun.ExtrudedPolygon.from_file(
        MuonGunGCD, padding=0
    )
    detector_boundaries_100 = MuonGun.ExtrudedPolygon.from_file(
        MuonGunGCD, padding=100
    )
    detector_boundaries_200 = MuonGun.ExtrudedPolygon.from_file(
        MuonGunGCD, padding=200
    )
...
def compute_skimming_event(
    ...
    intersections_100 = detector_boundaries_100.intersection(
        primary.pos, primary.dir
    )
    intersections_200 = detector_boundaries_200.intersection(
        primary.pos, primary.dir
    )

    int_one, int_two = intersections.first, intersections.second
    int_one_100, int_two_100 = (
        intersections_100.first,
        intersections_100.second,
    )
    int_one_200, int_two_200 = (
        intersections_200.first,
        intersections_200.second,
    )

    length, veto_size, skimming = compute_lengths(int_one, int_two)
    length_100, veto_size_100, skimming_100 = compute_lengths(
        int_one_100, int_two_100
    )
    length_200, veto_size_200, skimming_200 = compute_lengths(
        int_one_200, int_two_200
    )
...
from graphnet.data.extractors.icecube.utilities.vector_computations import (
    closest_approach_distance_vector,
)
...
def interaction_vertex_position(frame, leading_choice):
...
def get_topology_metrics(
    ...
    primary_high_rate = is_starting_visible(
        frame, int_vertex=primary_s, df=primary_df, optimzied=False
    )
    primary_low_rate = is_starting_visible(
        frame, int_vertex=primary_s, df=primary_df, optimzied=True
    )
    del primary_df
    leading_high_rate = is_starting_visible(
        frame, int_vertex=leading_s, df=leading_df, optimzied=False
    )
    leading_low_rate = is_starting_visible(
        frame, int_vertex=leading_s, df=leading_df, optimzied=True
    )
    del leading_df
    ...
    if (
        (
            (int_vert.pos.z <= max(dom_list.z))
            and (int_vert.pos.z >= min(dom_list.z))
        )
    ) and boundary.contains_points([(int_vert.pos.x, int_vert.pos.y)]):
