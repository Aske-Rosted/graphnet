...
def turn_mctree_into_light_sources(
    ...
    for particle in mctree:
        ...
        if particle.shape == dataclasses.I3Particle.ParticleShape.Primary:
            continue
        if (
            (np.abs(particle.pdg_encoding) == 12)
            | (np.abs(particle.pdg_encoding) == 14)
            | (np.abs(particle.pdg_encoding) == 16)
        ):
            continue
        if number_primaries > 1:
            if (
                mctree.get_primary(particle).minor_id != primary.minor_id
            ):  # Cleaning Coincidence Hits
                # if mctree.is_in_subtree(primary, particle) == False: # Cleaning Coincidence Hits
                continue
        if np.abs(particle.pdg_encoding) in [
            13,
            15,
        ]:  # Selecting Muons and Taus
            try:
                previous_sibling = mctree.previous_sibling(particle)
                stochastic_losses = []
                while (
                    np.abs(previous_sibling.pdg_encoding) not in [13, 15]
                ) or (
                    previous_sibling.shape
                    ...
                    try:
                        previous_sibling = mctree.previous_sibling(
                            previous_sibling
                        )
                    except:
                        break
                ...
                cont_loss = (
                    previous_sibling.energy - stoch_loss - particle.energy
                )
                loss_pos = (
                    previous_sibling.pos
                    + (particle.pos - previous_sibling.pos) / 2
                )
...
def turn_mcpe_into_light_sources(
    ...
    for _, dom in enumerate(doms_hit_by_different_particles):
        ...
        for particle in dom[1].keys():
            ...
            if number_primaries > 1:
                if (
                    mc_tree.get_primary(particle).minor_id != primary.minor_id
                ):  # Cleaning Coincidence Hits
    ...
    if len(mcpe_information) == 0:
        ...
        frame["fraction_coincidence"] = dataclasses.I3Double(
            coincidence_hits
            / (mcpe_information["MCPEs"].sum() + coincidence_hits)
        )
...
def compute_training_labels(
    ...
    try:
        hist, edges = np.histogram(
            s, bins=np.linspace(-1200, 1200, bin_count), weights=w
        )
        ...
        dict_training.update(
            {
                ...
                f"stochasticity_ratio_above_mean": np.max(hist)
                / np.mean(hist),
                f"stochasticity_ratio_above_median": np.max(hist)
                / np.median(hist),
            }
        )
    except:
    ...
    frame[f"{leading_type}ShowerProfile_{information_type}"] = (
        dataclasses.I3MapStringDouble(dict_training)
    )


def no_signal_hits(frame, information_type, leading_type="Primary"):
    """Stochasticity."""
    dict_training = {}
...
def make_shower_and_stochasticity_info(
    ...
    if len(mcpe_information) <= 5:
        ...
        no_signal_hits(frame, "MCPEs", leading_type="Primary")

        no_signal_hits(frame, "energy", leading_type="Primary")

        no_signal_hits(frame, "MCPEs", leading_type="Leading")

        no_signal_hits(frame, "energy", leading_type="Leading")
        ...
        frame["ShowerLengthDeposited"] = dataclasses.I3MapStringDouble(
            length_deposited
        )
